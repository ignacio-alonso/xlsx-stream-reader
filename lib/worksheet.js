...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetNode = function (nodeData) {
  ...
  if (self.inRows === true) {
    ...
    // The 'c' in the xml stands for 'cell'
    if (workingCell.name === 'c') {
      var cellNum = self.getColumnNumber(workingCell.attributes.r)

      /**
       * The rowCount is only incremented when any of the cells in the row has a value.
       * Note that colors or other formats are not considered values.
       * The last counted row is preserved to avoid counting it again.
       */
      if (workingVal && self.lastCountedRow != self.workingRow.attributes.r) {
        self.lastCountedRow = self.workingRow.attributes.r;
        self.rowCount++;
      }
      ...
      switch (workingCell.attributes.t) {
        case 's':
          // shared string. It has to get the value from the sharedString xml. workingVal contains the index instead of the real value.
