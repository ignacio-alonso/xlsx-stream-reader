    ...
    if (workingCell.name === 'c') {
      ...
      switch (workingCell.attributes.t) {
        ...
        case 'b': // eslint-disable-line no-fallthrough
          // boolean
          self.workingRow.values[cellNum] = Boolean(workingVal)
          workingCell = {}
          break
        case 'str':
          // string (formula)
...
XlsxStreamReaderWorkSheet.prototype._emitRow = function (rowData) {
  // Notice that row.values is a sparse array. The first element of each row and also any blank cells in between values are empty slots.
  // Empty slots and the empty cells are filtered out.
  // If the resulting array is empty after filtering, the entire row is ignored.
  if (!rowData.values.filter((value) => true && value !== '').length) return
  this.rowCount++
