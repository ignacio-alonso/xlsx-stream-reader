...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetNode = function (nodeData) {
  ...
  switch (nodeData[0].name) {
    ...
    case 'row': // eslint-disable-line no-fallthrough
      ...
      // When none of the row cells contain any characters (later called "workingPart"), the nodeData array has just one element: the row.
      // We don't count empty rows. Notice that colors or other formats are not considered here.
      if (nodeData.length > 1) ++self.rowCount
